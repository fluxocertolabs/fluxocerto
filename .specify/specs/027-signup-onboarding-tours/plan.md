...
- A **skippable onboarding wizard** that helps the user create the **minimum dataset for a projection** (account + income + expense), with **server-side persisted progress** and “Continue setup” re-entry points.
...
**Storage**: Supabase PostgreSQL (existing: `groups`, `profiles`, `accounts`, `projects`, `expenses`, `credit_cards`, …; new: onboarding/tour state tables)  
...
**Performance Goals**: On app load, add at most ~2 small reads (onboarding + tour state) beyond existing finance/group reads; keep UI overlays responsive (60fps)  
...
- **NFR-004 (Overlay coordination)**: Onboarding wizard and page tours must never overlap; tours are deferred while the wizard is open and may auto-start on the next eligible page visit after completion/dismissal.
...
| **Auth = invite-only allowlist** | ⚠️ EXCEPTION | Constitution states invite-only via allowlist; this feature explicitly changes the product to self-serve signup (see `spec.md` Assumptions + FR-002/FR-007). Plan disables allowlist gating while keeping RLS. |
...
**Gate Result**: ✅ PROCEED (1 explicit, spec-required exception documented)
...
├── pages/
│   ├── auth-callback.tsx                             # Modified: ensure provisioning before entering app
...
e2e/tests/
├── auth-self-serve.spec.ts                           # NEW: fresh-email signup should reach app w/o orphan state
...
> No unjustified complexity beyond a spec-required auth-model change (invite-only → self-serve).

| Violation | Why Needed | Simpler Alternative Rejected Because |
|-----------|------------|-------------------------------------|
| Invite-only access control (constitution fact) | Spec requires self-serve signup | Keeping allowlist would violate spec scope and block acquisition; hiding allowlist errors in UI would still prevent signup |
