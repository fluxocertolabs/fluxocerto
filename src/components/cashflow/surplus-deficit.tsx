...
export function SurplusDeficit({ optimistic, pessimistic }: SurplusDeficitProps) {
  const formatValue = (value: number) => {
    ...
    const label = isSurplus ? 'Superávit' : 'Déficit'
  ...
  return (
    ...
    >
      <p className="text-sm text-muted-foreground mb-3">Mudança Projetada</p>

      <div className="space-y-2">
        {/* Optimistic scenario */}
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted-foreground">Melhor cenário:</span>
          <span
            className={cn(
              'font-semibold',
        ...
        {!isSameScenario && (
          <div className="flex items-center justify-between">
            <span className="text-sm text-muted-foreground">Pior cenário:</span>
            <span
              className={cn(
                'font-semibold',
